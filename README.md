...
### <a id="sec-api-end-to-end-tests"></a>Teste REST API End-to-End
...
É uma coisa linda poder escrever testes unitários para toda a sua base de código, independentemente da camada da arquitetura da sua aplicação em que você está. O exemplo mostra um teste unitário simples para um controlador. Infelizmente, quando se trata dos controladores do Spring, há uma desvantagem nesta abordagem: os controladores do Spring MVC fazem uso intenso de anotações para declarar quais caminhos estão ouvindo, quais verbos HTTP usar, quais parâmetros eles analisam do caminho da URL ou parâmetros de consulta e assim por diante. Simplesmente invocar o método de um controlador dentro dos seus testes unitários não testará todas essas coisas cruciais. Felizmente, os desenvolvedores do Spring criaram um auxiliar de teste útil que você pode usar para escrever melhores testes de controlador. Certifique-se de conferir o [MockMVC](https://docs.spring.io/spring-framework/reference/testing/spring-mvc-test-framework/server.html). Ele oferece uma DSL agradável que você pode usar para disparar solicitações falsas contra o seu controlador e verificar se está tudo bem. Eu incluí um [exemplo](https://github.com/hamvocke/spring-testing/blob/master/src/test/java/example/ExampleControllerAPITest.java) na base de código de amostra. Muitos frameworks oferecem auxiliares de teste para tornar o teste de aspectos específicos da sua base de código mais agradável. Confira a documentação do seu framework de escolha e veja se ele oferece algum auxiliar útil para seus testes automatizados.
